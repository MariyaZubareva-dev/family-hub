...
export default function App(){
 ...
 const [busy,setBusy]=useState(true); const [error,setError]=useState<string|null>(null); const [eventForm,setEventForm]=useState(emptyEvent()); const [showEvent,setShowEvent]=useState(false); const [reminderForm,setReminderForm]=useState<ReminderForm>(emptyReminder()); const [showReminder,setShowReminder]=useState(false); const [selectedEvent,setSelectedEvent]=useState<EventItem|null>(null); const [newItem,setNewItem]=useState(''); const [newNote,setNewNote]=useState({title:'',body:''}); const [newIdea,setNewIdea]=useState({title:'',description:''}); const [newFinance,setNewFinance]=useState<FinanceForm>({type:'EXPENSE',amount:'',category:'',budget_type:'FLEXIBLE',description:'',occurred_on:todayString()});
 ...
 </main><nav className="bottom-nav"><NavItem a={section==='home'} l="Главная" i="⌂" c={()=>setSection('home')}/><NavItem a={section==='calendar'} l="Календарь" i="□" c={()=>setSection('calendar')}/><NavItem a={section==='lists'} l="Списки" i="☷" c={()=>setSection('lists')}/><NavItem a={section==='more'} l="Ещё" i="•••" c={()=>setSection('more')}/></nav>{showEvent&&<EventSheet form={eventForm} setForm={setEventForm} members={family?.members??[]} onClose={()=>setShowEvent(false)} onSubmit={createEvent} busy={busy}/>} {showReminder&&<ReminderSheet form={reminderForm} setForm={setReminderForm} members={family?.members??[]} onClose={()=>setShowReminder(false)} onSubmit={createReminder} busy={busy}/>} {selectedEvent&&<EventDetails event={selectedEvent} onClose={()=>setSelectedEvent(null)}/>}</div>
}
