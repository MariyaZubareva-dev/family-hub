...
export default function App(){
 ...
 const [busy,setBusy]=useState(true); const [error,setError]=useState<string|null>(null); const [eventForm,setEventForm]=useState(emptyEvent()); const [showEvent,setShowEvent]=useState(false); const [reminderForm,setReminderForm]=useState<ReminderForm>(emptyReminder()); const [showReminder,setShowReminder]=useState(false); const [showFamily,setShowFamily]=useState(false); const [inviteForm,setInviteForm]=useState<InviteForm>({telegram_user_id:'',username:'',first_name:''}); const [selectedEvent,setSelectedEvent]=useState<EventItem|null>(null); const [showItemSheet,setShowItemSheet]=useState(false); const [newItem,setNewItem]=useState(''); const [newNote,setNewNote]=useState({title:'',body:''}); const [newIdea,setNewIdea]=useState({title:'',description:''}); const [newFinance,setNewFinance]=useState<FinanceForm>({type:'EXPENSE',amount:'',category:'',budget_type:'FLEXIBLE',description:'',occurred_on:todayString()});
 ...
 const inviteMember=async()=>{if(!inviteForm.telegram_user_id.trim()||!family)return;try{setBusy(true);await post('/family/invitations',{telegram_user_id:Number(inviteForm.telegram_user_id),username:inviteForm.username||null,first_name:inviteForm.first_name||null});setInviteForm({telegram_user_id:'',username:'',first_name:''});setError('Приглашение отправлено.');}catch(e){setError(e instanceof Error?e.message:'Не удалось отправить приглашение.')}finally{setBusy(false)}};
 const updateMemberRole=async(memberId:string,role:'ADMIN'|'USER')=>{try{setBusy(true);await patch(`/family/members/${memberId}`,{role});await reload()}catch(e){setError(e instanceof Error?e.message:'Не удалось изменить роль.')}finally{setBusy(false)}};
 const removeMember=async(memberId:string)=>{try{setBusy(true);await del(`/family/members/${memberId}`);await reload()}catch(e){setError(e instanceof Error?e.message:'Не удалось удалить участника.')}finally{setBusy(false)}};
